...
import { addListToModel } from "../Controller/listController";
export const addList = (parent) => {
  ...
  addButton.addEventListener("click", (event) => {
    ...
    document.querySelectorAll("#todoItems > h5").forEach((element) => {
      if (element.ariaSelected === "true") {
        addListToModel(element.textContent, inputDescription.value);
      }
    });

    createEl("div", inputDescription.value, "basis-full border-b-2", list);
    modal.classList.add("hidden");
  });
